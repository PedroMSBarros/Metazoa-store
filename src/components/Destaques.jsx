    ...
    <section id="destaques" className="py-20 px-6 bg-[#F4F1E1]">
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div className="flex justify-between items-end mb-12 flex-wrap gap-4" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <a href="https://wa.me/5511999999999" target="_blank" rel="noreferrer" className="text-[#5B8C7A] text-sm font-medium flex items-center gap-1 hover:gap-2 transition-all">
        ...
        {carregando ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {peixes.map((peixe, i) => (
              <motion.div key={peixe.id} className="bg-white rounded-xl overflow-hidden hover:-translate-y-1 transition-transform duration-300 shadow-sm hover:shadow-md" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}>
