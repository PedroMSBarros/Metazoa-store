    ...
    <section id="destaques" className="py-20 px-6 bg-[#F4F1E1]">
      <div className="max-w-6xl mx-auto">
        <motion.div
          ...
          <Link
            to="/catalogo"
            className="text-[#5B8C7A] text-sm font-medium flex items-center gap-1 hover:gap-2 transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {peixes.map((peixe, i) => (
              <motion.div
                ...
                <Link
                  to={"/peixe/" + peixe.id}
                  className="bg-white rounded-xl overflow-hidden hover:-translate-y-1 transition-transform duration-300 shadow-sm hover:shadow-md block">
                  <div className="relative aspect-[4/3] overflow-hidden bg-[#E8E3CC]">
                    ...
                    />
                    {peixe.badge && (
                      <span className="absolute top-3 left-3 bg-[#5B8C7A] text-white text-xs font-medium px-3 py-1 rounded-full">
                        {peixe.badge}
                      </span>
                    )}
                  </div>
                  <div className="p-5">
                    <span className="text-xs font-medium tracking-widest uppercase text-[#9C8A6A] block mb-1">
                      {peixe.categoria}
                    </span>
                    <div className="font-serif text-xl text-[#2C2416] mb-1">{peixe.nome}</div>
                    <span className="font-serif italic text-sm text-[#7A6A52] block mb-3">
                      {peixe.nome_cientifico}
                    </span>
                    <div className="flex justify-between items-center pt-3 border-t border-[#E8E3CC]">
                      ...
                      <span className="bg-[#5B8C7A] text-white text-sm px-4 py-2 rounded">
                        Ver detalhes
                      </span>
                    </div>
                  </div>
