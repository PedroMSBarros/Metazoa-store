...
function Catalogo() {
  ...
  const peixesFiltrados = filtro === 'Todos'
    ? peixes
    : aguaDoceValues.includes(filtro) && filtro === 'Agua Doce'
    ? peixes.filter(p => aguaDoceValues.includes(p.categoria))
    : peixes.filter(p => p.categoria === filtro)

  function handleFiltro(value) {
    setFiltro(value)
    if (value !== 'Todos' && !aguaDoceValues.includes(value)) {
        ...
        {carregando ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {peixesFiltrados.map((peixe, i) => (
              <motion.div key={peixe.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.05 }}>
