...
import { ArrowRight, Timer, Beaker } from 'lucide-react'
...
const guias = [
  {
    icon: Timer,
    titulo: 'Guia de Aclimatação',
    texto: 'Passo a passo completo para introduzir peixes novos no aquário com segurança.',
    link: '/cuidados/aclimatacao'
  },
  {
    icon: Beaker,
    titulo: 'Montagem e Ciclagem',
    texto: 'Como montar seu aquário do zero e entender o ciclo do nitrogênio.',
    link: '/cuidados/montagem'
  },
]
  ...
  return (
    <div className="bg-[#F4F1E1] min-h-screen">
      ...
      <div className="pt-24 pb-20 px-6 max-w-6xl mx-auto">
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-14" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
          {guias.map((guia) => (
            <Link key={guia.titulo} to={guia.link} className="bg-[#2C1A0E] rounded-2xl p-6 flex items-center gap-5 hover:bg-[#3A2510] transition-colors group">
              <div className="w-14 h-14 rounded-full bg-[#4A8C1C]/20 flex items-center justify-center flex-shrink-0">
                <guia.icon className="text-[#4A8C1C]" size={26} />
              </div>
              <div className="flex-1">
                <h3 className="font-serif text-xl text-[#C8D4A0] mb-1">{guia.titulo}</h3>
                <p className="text-sm text-[#C8D4A0]/60">{guia.texto}</p>
              </div>
              <ArrowRight className="text-[#C8D4A0]/40 group-hover:text-[#4A8C1C] group-hover:translate-x-1 transition-all flex-shrink-0" size={20} />
            </Link>
          ))}
        </motion.div>
        ...
        <motion.div className="mt-16 bg-[#2C1A0E] rounded-2xl p-8 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }}>
          ...
          <a href="https://wa.me/5511971526750?text=Ola! Tenho duvidas sobre cuidados com meu aquario." target="_blank" rel="noreferrer" className="bg-[#25D366] text-white px-8 py-3 rounded-full font-medium inline-flex items-center gap-2 hover:bg-[#1ebe5d] transition-colors">
